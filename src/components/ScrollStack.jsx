...
const ScrollStack = ({
  ...
  // Cached offsets to prevent layout thrashing on scroll
  ...
  const refreshOffsets = useCallback(() => {
    ...
    if (endElement) {
      if (useWindowScroll) {
        ...
        endOffset = endElement.offsetTop;
      }
    }

    // Batch write: restore transforms
  ...
  useLayoutEffect(() => {
    ...
    // Compute original offsets once at mount
    refreshOffsets();
    ...
    // Recalculate offsets on window resize or load
    ...
    return () => {
      ...
      window.removeEventListener('load', refreshOffsets);
