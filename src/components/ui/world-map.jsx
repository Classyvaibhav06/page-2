...
/**
 * WorldMap — adapted from the Aceternity UI component for Vite + React (no Next.js).
 * Replaces next/image with a plain <img> and removes next-themes dependency.
 */
export function WorldMap({
  ...
  return (
    <div className="w-full aspect-[2/1] rounded-2xl relative font-sans overflow-hidden" style={{ background: "#FAF9F6" }}>
      {/* Dotted base map */}
      <img
      ...
      >
        <defs>
          <linearGradient id="path-gradient-maroon" x1="0%" y1="0%" x2="100%" y2="0%">
          ...
          return (
            <g key={`path-group-${i}`}>
              <motion.path
                ...
                fill="none"
                stroke="url(#path-gradient-maroon)"
                strokeWidth="1.5"
      ...
      <AnimatePresence>
        {hoveredLocation && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-4 left-4 px-3 py-2 rounded-lg text-xs font-semibold sm:hidden"
            style={{
              background: "rgba(255,255,255,0.95)",
              color: "#1c1917",
              border: "1px solid #d6d3d1",
